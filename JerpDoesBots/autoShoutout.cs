				...
				if (shoutEntry.name.ToLower() == shoutUserEntry.Nickname.ToLower())
				{
					if (!shoutEntry.shoutedSinceLoad || m_BotBrain.actionTimer.ElapsedMilliseconds > (shoutEntry.lastShouted + shoutThrottle))
					{
						...
						if (channelInfo != null && !string.IsNullOrEmpty(channelInfo.Game))
							lastGame = "  " + string.Format(m_BotBrain.Localizer.getString("shoutoutLastPlaying"), channelInfo.Game);
						...
						if (!string.IsNullOrEmpty(shoutEntry.shoutMessage))
						{
                            switch(shoutEntry.type)
                            {
                                ...
                                case autoShoutUserType.streamer:
									m_BotBrain.sendDefaultChannelMessage(string.Format(m_BotBrain.Localizer.getString("shoutoutMessage"), channelInfo.DisplayName, shoutEntry.shoutMessage, channelInfo.DisplayName.ToLower()) + lastGame);
									break;
                            }
						...
						{
							m_BotBrain.sendDefaultChannelMessage(string.Format(m_BotBrain.Localizer.getString("shoutoutMessage"), channelInfo.DisplayName, channelInfo.DisplayName.ToLower()) + lastGame);
						}
